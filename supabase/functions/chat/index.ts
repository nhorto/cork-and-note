...
export function createHandler(overrides: Partial<HandlerDeps> = {}) {
  ...
  return async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      let parsed: { messages?: unknown; system_prompt?: unknown; task?: unknown; stream?: unknown };
      try {
        parsed = JSON.parse(raw);
      } catch {
        return json({ error: "Invalid JSON body" }, 400);
      }
      const { messages, system_prompt, task, stream } = parsed;
      ...
      // Ordinary conversation can opt into newline-delimited events. The first
      // upstream request is opened before returning 200 so auth, entitlement,
      // and immediate Anthropic failures still retain their proper HTTP status.
      // Structured tasks stay JSON because their callers need an atomic fenced
      // block before updating a form.
        ...
        const eventStream = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              // An admitted streaming call may already have generated billable
              // tokens even if its connection breaks. Count it so repeatedly
              // disconnecting cannot bypass the free/fair-use meter.
              await recordUsage();
              try {
                emit({
                  type: "error",
                ...
                // The reader itself disconnected; there is nobody left to notify.
              }
            } finally {
