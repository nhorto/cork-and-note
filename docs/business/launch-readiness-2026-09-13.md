...
**September 15 update.** Live state was checked through the App Store Connect, Google Play and Supabase APIs. iOS **27** was the selected build for version 1.0, but it cannot be submitted: its password reset fails on every warm launch (PR #315, merged the same day), so **build 28 is required**. The Google Play **closed (alpha) test is live** with Android 7; Google approved the September 14 submission. The Meta Android pilot is approved and active with no spend recorded yet, and zero early-access requests so far. The [App Review audit](../audits/2026-09-15-app-review-audit.md) fixed the missing category, price, availability and content-rights declarations, added a Free reviewer account and corrected the review notes; three small code follow-ups ship in build 28 alongside the reset fix. Signup now sends an acknowledgement and an owner alert (PR #312), and corkandnote.com is attached to the Vercel project. Owner items before Submit: the App Privacy label additions (Name, Purchase History), the annual sandbox purchase and a Restore on a physical iPhone. Android: 12 opted-in closed testers for 14 days, which needs a tester list or Google Group configured in Play Console, then the join link set as `EARLY_ACCESS_ANDROID_JOIN_URL` in Vercel.
...
**Required next step:** use the [device acceptance worksheet](device-acceptance-2026-09-13.md) to perform acceptance on **iOS 27 / Android 7**, including the redesigned Home/Sommelier, badges and follow-up fixes. Recheck marketing screenshots against the updated UI. The earlier 25/6 archives remain historical only. [Integration evidence](../audits/2026-09-13-pr-integration.json); [current candidate/auth evidence](../audits/2026-09-13-candidates-and-auth.json).
...
**Latest Android 7 follow-through:** reviewer login, badge recognition, map tiles/clusters, AVA boundaries, Napa directory search/detail and cluster expansion were observed. A planned five-minute Napa/Sonoma stress run stopped when an ADB swipe exceeded 20 seconds. Seven memory samples over 94 seconds rose from about 318 to 384 MiB PSS; all sampled app process IDs matched. Logs identified a **launcher ANR**, with no Cork & Note crash/ANR identified in the captured buffers. The recenter control remained busy, so recenter acceptance was not established. Final screen showed an unsaved pin dialog; no pin was saved. This is **inconclusive**, not a map-stability pass or proof of an app root cause. The dedicated emulator was stopped. [Sanitized follow-through evidence](../audits/2026-09-13-acceptance-followthrough.json).

**Four-core emulator repeat:** a complete 300-second Napa/Sonoma pan exercise finished with 40 gestures and the same app PID in all 20 samples. No crash/ANR was recorded in the captured buffers. PSS rose from 408,812 to 534,777 KiB (about 399 to 522 MiB); this is not a bounded-memory pass. The layer sheet opened after the run and turning regions off visibly removed AVA polygons. A subsequent 15-second harness smoke after restoring layers sampled about 589 MiB PSS, reinforcing the need to investigate growth. Recenter remained disabled and is not accepted. Issue #265 remains open pending memory investigation and physical-device acceptance. [Repeat evidence](../audits/2026-09-13-map-repeat.json). The reusable `scripts/qa-map-stress.py` preserves partial receipts on failure and requires a dedicated emulator serial; manual visual/log review is still required.
...
- [x] **Engineering: fix failure-state observations in source.** Taste reads now show retryable failures and preserve saved reports; settings provider copy matches Gemini scans and Anthropic chat. Regression tests pass. These fixes are included in iOS 27 / Android 7; physical-device acceptance remains.
...
- [ ] **Owner: verify the support mailbox and response process.** The app, public support/deletion pages and Play setup already use `cork_and_note@yahoo.com`; the remaining check is that it is monitored. Support/deletion-request handling and a routine for AI reports need an accountable operator. Reviewer login/Pro are verified; Google Console app-access instructions still need confirmation.
