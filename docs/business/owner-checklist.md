...
| **Blocking a real launch** | **Custom SMTP** (§C). The shared Supabase sender allows 2 emails/hour project-wide, so password resets start silently failing the moment more than a couple of people need one. |
...
- [x] **Switch the annual trial from 7 days to 3 days** — ✅ done 2026-09-09, via the App Store Connect API (deleted the 175 per-territory 7-day free-trial offers on `pro_annual`, recreated all 175 as 3-day free trials, verified). The app and paywall need no change: they read the trial length from StoreKit at runtime.
...
- [ ] **Transactional email (custom SMTP)** — ⚠️ **a launch blocker in its own right, not just branding.** Auth mail currently goes through Supabase's shared sender, which is capped at **2 emails per hour for the entire project** and which Supabase documents as unsuitable for production. Three people forgetting their password in the same hour means the third gets nothing. It is also why the emails still look like generic Supabase mail from `noreply@mail.app.supabase.io`, and why template edits are refused outright ("not available for free tier projects using the default email provider").
  **You do not need Supabase Pro for this** — any SMTP provider unlocks all three at once. [Resend](https://resend.com) is free to 3,000 emails/month: create an account, add the domain, paste the DNS records at the registrar, then hand back the SMTP host/user/password. It needs a verified sending domain, so it waits on the domain purchase below.
  *Hand back:* the SMTP credentials. Then `node scripts/auth-emails.mjs --push` puts the branded Royal Velvet templates live (they are written and committed already). Background: [`docs/audits/2026-09-10-auth-flow.md`](../audits/2026-09-10-auth-flow.md).
