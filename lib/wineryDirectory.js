...
// Freshness filter (#225, #271): hide wineries we KNOW are gone and rows
// flagged as duplicates of another. NULL means "unknown" — the overwhelming
// majority — and must be kept, so this can't be a bare .neq() (PostgREST
// `neq` evaluates NULL rows to NULL → filtered out). 'possibly_closed'
// (missing from the latest Overture extract) and 'temporarily_closed' still
// show: absence from one release isn't proof of closure, and a temporary
// closure is worth knowing about on the page rather than hiding.
const NOT_PERMANENTLY_CLOSED =
  'operating_status.is.null,operating_status.not.in.(permanently_closed,duplicate)';
