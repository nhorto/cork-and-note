...
export const chatService = {
  ...
  async addMessage(
    ...
    // `sources` was added after the original messages table. Avoid sending an
    // empty value so chat keeps working while that migration rolls out, and
    // retry cited replies without it when an older database reports PGRST204.
    // The reply can still show citations in the current session; persistence
    // starts automatically as soon as the migration is installed.
    const insertMessage = (body) => supabase
      .from('messages')
      .insert(body)
      .select()
      .single();
    ...
    if (normalizedSources.length > 0 && this._isMissingSourcesColumn(error)) {
      ({ data, error } = await insertMessage(payload));
    }

    if (error) throw error;
    return {
      ...data,
      sources: Array.isArray(data?.sources) ? data.sources : normalizedSources,
    };
  },

  _isMissingSourcesColumn(error) {
    if (!error) return false;
    return error.code === 'PGRST204'
      && /(?:column|field)[^]*\bsources\b|\bsources\b[^]*(?:column|field)/i.test(error.message || '');
  },
