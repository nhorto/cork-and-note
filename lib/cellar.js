...
export const cellarService = {
  ...
  async openBottle(bottleId, {
    ...
    try {
      ...
      // history, and — when a tasting was logged — the visits list. Invalidate
      // them all here (one place) before the final quantity update / reload;
      // nothing re-reads these caches in between, so the keys stay empty until
      // the next live fetch.
      invalidate(
        CACHE_KEYS.cellar,
      ...
      if (keepBottle) {
        ...
        return { success: true, bottle: annotate(same), wineId, visitId, keptBottle: true };
      }
      ...
      return { success: true, bottle: annotate(updated), wineId, visitId, keptBottle: false };
    } catch (error) {
