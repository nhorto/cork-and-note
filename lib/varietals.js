...
// (Red / White / …) from a chosen varietal. Ambiguous named blends ("Bordeaux
// Blend" can be red or white, etc.) infer nothing — the user picks the type;
// unambiguous ones (Red Blend, GSM, …) join inference via BLEND_TYPE below.
// Whole-list, blend-aware inference lives in inferTypeFromVarietals (#216).
...
// Exported for the badge catalog (#295), which has to decide whether two
// differently spelled grapes ("Pinot Gris", "pinot-gris") are the same one.
export function varietalKey(value) {
...
// Named blends whose colour IS unambiguous, so they can join type inference.
// Bordeaux Blend / Meritage / Rhône Blend / Field Blend stay out — each has a
// real white variant, and guessing wrong is worse than not filling the field.
const BLEND_TYPE = {
  'red blend': 'Red Blend',
  'white blend': 'White Blend',
  'rosé blend': 'Rosé',
  'gsm': 'Red Blend', // Grenache-Syrah-Mourvèdre is red by definition
};

// Blend-aware type inference over the WHOLE varietal list (#216): one red grape
// → "Red", two or more → "Red Blend", same for whites. Sparkling or dessert
// anywhere in the list wins outright (Champagne + Chardonnay is still
// Sparkling). Mixed red+white returns null — could be a rosé, an orange wine,
// or a field blend, so the user decides. Only recognised grapes count toward
// the blend arithmetic; an unknown local grape neither infers nor blocks.
export function inferTypeFromVarietals(value) {
  const types = parseVarietals(value)
    .map((g) => TYPE_BY_VARIETAL[g.toLowerCase()] || BLEND_TYPE[g.toLowerCase()] || null)
    .filter(Boolean);
  if (types.length === 0) return null;
  if (types.includes('Sparkling')) return 'Sparkling';
  if (types.includes('Dessert')) return 'Dessert';
  const reds = types.filter((t) => t === 'Red' || t === 'Red Blend');
  const whites = types.filter((t) => t === 'White' || t === 'White Blend');
  const rosés = types.filter((t) => t === 'Rosé');
  if (reds.length === types.length) {
    return types.length > 1 || reds.includes('Red Blend') ? 'Red Blend' : 'Red';
  }
  if (whites.length === types.length) {
    return types.length > 1 || whites.includes('White Blend') ? 'White Blend' : 'White';
  }
  if (rosés.length === types.length) return 'Rosé';
  return null; // mixed colours — no safe guess
}
