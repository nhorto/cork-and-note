...
export function regionSuggestions(userRegions = []) {
  const out = [];
  const seen = new Set();
  const add = (name, aliases, source) => {
    const key = regionKey(name);
    if (!key || seen.has(key)) return;
    seen.add(key);
    out.push({ name, aliases, subtitle: regionSubtitle(name), source });
  };
  ...
  for (const entry of userRegions) {
    ...
    add(reference ? reference.name : typed, reference ? reference.aliases : [], 'cellar');
  }
  for (const region of WINE_REGIONS) add(region.name, region.aliases, 'reference');
