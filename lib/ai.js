...
// The most any caller may ask for (see photoToBase64's maxEdge). Anthropic
// downsamples above ~1,568px on the long edge anyway, so past this we would be
// paying upload time for pixels the model never sees.
...
Only include the wine_suggestions block when you have specific wine information to suggest. Always include a conversational response before the block.${context}`;
  ...
  parseFencedJson(responseText, tag) {
    ...
    const open = responseText.indexOf('```' + tag);
    if (open === -1) {
