...
const key = (userId) => `ai-sharing-consent:v1:${userId}`;
const pending = new Map();
const listeners = new Set();

export const AI_SHARING_DISCLOSURE = 'To provide AI suggestions, Cork & Note sends your messages, photos you scan or attach, and relevant tasting notes and cellar details to Anthropic. Pro can also search the web: queries based on your conversation go to Anthropic’s search provider. AI can make mistakes. You can turn this off in Account settings and keep using your journal without AI.';

export async function getAiConsent(userId) {
  if (!userId) return false;
  const value = await AsyncStorage.getItem(key(userId));
  return value === 'allowed' ? true : value === 'declined' ? false : null;
}
