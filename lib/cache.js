...
export const CACHE_KEYS = {
  ...
  // Bottles + their consumption history in one payload, for the badge facts.
  // Separate from `consumptions`, which holds a flat list of pours in a
  // different shape (lib/cellarInsights.js) and would collide under one key.
  cellarHistory: 'cellarHistory',
...
const generations = new Map(); // key -> int, bumped by invalidate() so a read
// that started BEFORE a mutation can't write its stale result back afterwards.
 ...
 *
 * Failure shapes are NOT cached: our services resolve `{ success: false }` on
 * handled errors instead of throwing, and pinning that for the TTL made an
 * offline blip look like an empty account for 5 minutes (July 2026 audit). A
 * failed read is returned to the caller but the next call re-fetches. Thrown
 * errors are likewise never pinned.
 *
 * A read that was already in flight when invalidate() ran does not write its
 * (pre-mutation, stale) result into the store — that's what the generation
 * counter guards.
 *
...
export async function cached(key, fn, ttlMs = DEFAULT_TTL_MS) {
  ...
  log('MISS', key);
  const genAtStart = generations.get(key) || 0;
  const promise = (async () => {
    const value = await fn();
    const failed = value && typeof value === 'object' && value.success === false;
    const invalidatedMeanwhile = (generations.get(key) || 0) !== genAtStart;
    if (!failed && !invalidatedMeanwhile) {
...
export function invalidate(...keys) {
  for (const key of keys) {
    store.delete(key);
    // Also forget any in-flight read and mark the generation so a read that
    // started before this mutation can't re-populate the store with stale data.
    inflight.delete(key);
...
export function clearAll() {
  store.clear();
  inflight.clear();
  // Forgetting the in-flight promise is not enough: the read behind it is still
  // running and would write its result into the store when it resolves, which
  // after a sign-out then sign-in is the PREVIOUS account's rows served to the
  // new one (__tests__/cache.test.js). Bumping every generation makes that
  // late write a no-op, the same way invalidate() protects a single key.
  for (const key of new Set([...Object.values(CACHE_KEYS), ...generations.keys()])) {
    generations.set(key, (generations.get(key) || 0) + 1);
  }
