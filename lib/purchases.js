...
function apiKey() {
  ...
  // Each store has its own public SDK key (`appl_…` / `goog_…`). A platform
  // whose key is unset — or web, which has neither — returns null, and the app
  // runs without the Pro tier rather than crashing.
  if (Platform.OS === 'ios') return extra.revenueCatIosKey || null;
  if (Platform.OS === 'android') return extra.revenueCatAndroidKey || null;
  return null;
}
...
export function addProStatusListener(listener) {
  ...
  if (!Purchases || !configured) return () => {};
  try {
    const handler = (info) => listener(proFromCustomerInfo(info));
    const remove = Purchases.addCustomerInfoUpdateListener(handler);
    // The SDK returns an unsubscribe on some versions and nothing on others; a
    // listener we cannot remove would keep firing into an unmounted provider.
...
export async function fetchOffering() {
  // The paywall can mount before ProProvider finishes configuring the SDK.
  // Wait here too, so that race cannot strand the screen with an empty result.
  if (!(await configurePurchases())) {
    const error = new Error('Plans are not configured in this build. Please contact support.');
    error.code = 'PLANS_NOT_CONFIGURED';
    throw error;
  }
  const Purchases = getSdk();
  try {
    ...
    if (!offering?.availablePackages?.length) {
      const error = new Error('Plans could not be loaded. Please try again shortly.');
      error.code = 'NO_STORE_PLANS';
      throw error;
    }
    return offering;
  } catch (err) {
    console.warn('RevenueCat getOfferings failed:', err?.message);
    const error = new Error('Plans could not be loaded. Check your connection and try again.');
    error.code = err?.code === 'NO_STORE_PLANS' ? err.code : 'STORE_PLANS_ERROR';
    throw error;
  }
}

/**
 * Only show trial copy when the store confirms this customer's eligibility.
 *
 * The two stores answer this differently. On iOS the SDK asks StoreKit per
 * product (`checkTrialOrIntroductoryPriceEligibility`). On Android that call
 * ALWAYS returns UNKNOWN: Google Play does the filtering itself and only returns
 * offers the user is eligible for, and the SDK folds the default option's free
 * phase into `product.introPrice`. So on Android an `introPrice` being present
 * IS the eligibility signal; asking the iOS API there would hide the trial from
 * every Android customer.
 */
export async function fetchTrialEligibility(packages) {
  const Purchases = getSdk();
  if (!Purchases || !configured) return {};
  const identifiers = packages.map((pkg) => pkg.product.identifier);
  if (Platform.OS === 'android') {
    return Object.fromEntries(
      packages.map((pkg) => [pkg.product.identifier, Boolean(pkg.product.introPrice)])
    );
  }
  try {
    const statuses = await Purchases.checkTrialOrIntroductoryPriceEligibility(identifiers);
    return Object.fromEntries(identifiers.map((id) => [id, statuses[id]?.status === 2]));
  } catch {
    // Unknown eligibility must never promise a free trial.
    return {};
  }
}
