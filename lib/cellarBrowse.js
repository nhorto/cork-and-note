...
export const EMPTY_FILTERS = {
  ...
  locations: [], // storage location values
...
function matchesFilters(bottle, filters) {
  ...
  if (
    ...
    !filters.locations.some((l) => eqInsensitive(l, bottle.location || ''))
  ) {
    return false;
  }
...
export function facetOptions(bottles) {
  const collect = (fn) => {
    // Keyed by lower-case so "Napa Valley" and "napa valley" are ONE facet. This
    // has to match matchesFilters, which compares with eqInsensitive — when the
    // key was case-sensitive the two spellings produced two chips that selected
    // the identical set of bottles (#88).
    ...
    return [...set.values()]
      .map(({ count, spellings }) => ({
        // Show the spelling the user uses most. Ties fall to the first seen,
        // since Map preserves insertion order — so the label stays stable
        // rather than flickering between variants as the set is re-narrowed.
  ...
  return {
    ...
    locations: collect((b) => b.location),
  };
}
...
function groupKeyFor(bottle, groupBy) {
  switch (groupBy) {
    ...
    case 'location':
      return bottle.location?.trim() || UNKNOWN;
    case 'drink_by':
