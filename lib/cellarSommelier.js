...
// the canonical drink-window taxonomy) — it never writes to or modifies that file.
import { aiService } from './ai';
import { READY_STATUSES, cellarService, drinkWindowPrompt, drinkWindowStatus } from './cellar';
...
function buildCellarSystemPrompt(bottles) {
  const ready = bottles.filter((b) =>
    READY_STATUSES.includes(b.drinkStatus ?? drinkWindowStatus(b.drink_from, b.drink_by, b.vintage))
  ).length;
...
export const cellarSommelier = {
  ...
  async getTonightsPick(bottles, selections = {}) {
    try {
      ...
      // task 'tonights_pick' → the server's Pro-only meter (limit 0 free);
      // model selection still falls through to the default chat model.
      ...
      return {
        success: true,
        recommendation,
        // Strip the ```tonights_pick fence so the card shows just the friendly
        // lead-in, not the raw JSON. Uses the local helper to stay symmetric
        // with the failure path above (getDisplayText would also work now that
        // it strips this tag, but keeping both paths identical is clearer).
        raw: stripPickBlock(responseText),
      };
    } catch (err) {
