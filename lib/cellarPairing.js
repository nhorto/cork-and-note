...
How to advise:
- Be specific and bottle-aware. Reference this wine's actual style, structure, age, and region — never generic filler like "pairs with chicken." Name concrete dishes ("herb-crusted rack of lamb", "mushroom risotto with thyme"), not just ingredients.
- Tailor to the wine's likely body, tannin, acidity, and sweetness, and to its drink-window maturity if known.
- Honor the user's occasion / season when given.
- Offer a small, varied set (3–5 pairings) spanning categories where it makes sense — e.g. a main, something lighter or a starter, a cheese, maybe a vegetarian or dessert option — so there's a real choice, not five steaks.
...
function buildUserRequest(bottle = {}, { occasion, season, freeText } = {}) {
  ...
  return `${preamble}${context}\n\nReply with the food_pairing JSON block, with specific, bottle-aware dishes and reasoning.`;
}
...
// Strip the food_pairing code block (and any stray fenced JSON) so only the
// model's friendly sentence remains for the card's "aside" line. getDisplayText
// only strips the wine_suggestions block, so it would otherwise leak the raw
// food_pairing JSON into the UI. Mirrors stripLabelBlock in lib/cellarScan.js.
function stripPairingBlock(responseText) {
  if (!responseText) return '';
  return responseText
    .replace(/```(?:food_pairing)?\s*\n?[\s\S]*?```/g, '')
    .trim();
}
  ...
  async getPairings(bottle, selections = {}) {
    try {
      ...
      return {
        success: true,
        pairing,
        raw: stripPairingBlock(responseText),
      };
    } catch (err) {
