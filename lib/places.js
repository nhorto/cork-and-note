...
export const placesService = {
   ...
   * weekday hours, website, phone, Google Maps link, first photo name, and
   * businessStatus ("Permanently closed" badge, #225).
   ...
   *
   * `directoryId` (optional): the winery_directory row this page came from,
   * when known. The Edge Function uses it to write Google's businessStatus
   * back to the exact directory row (and to attach the place id there), so
   * future users' discovery pins reflect a closure.
   */
  async getDetails(placeId, { directoryId = null } = {}) {
    ...
    try {
      const body = { mode: 'details', place_id: placeId };
      if (directoryId != null) body.directory_id = Number(directoryId);
  ...
  /**
   * Match one of OUR winery records to a Google place (guarded name/location search,
