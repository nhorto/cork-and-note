...
// forwards to Gemini for structured extraction. We ask for a parseable ```cellar_label
...
- varietal: the grape(s) only if printed, else null.
- region: the region / appellation / country if printed (e.g. "Napa Valley", "Bordeaux", "Barolo DOCG"), else null. Give any COUNTRY name in English: a label reading "Italia" or "España" must come back as "Italy" or "Spain", so the same country never lands in the cellar under two spellings. Leave region and appellation names exactly as printed ("Barolo", "Rioja", "Saint-Émilion Grand Cru") — those are the wine's actual name, not something to translate.
...
- varietal: the grape(s) for that wine only if printed, else null.
- region: the region / appellation / country printed for that wine, else null. Give any COUNTRY name in English ("Italia" → "Italy", "España" → "Spain") so one country never lands in the cellar under two spellings; leave region and appellation names exactly as printed.
  ...
  async scanTastingCard(image) {
    try {
      ...
      const aiResponse = await aiService.sendMessage(
        ...
        systemPrompt,
        { task: 'tasting_menu_scan' } // separate output budget; shared label-scan meter
      );
      const responseText = aiResponse?.response || '';
...
export async function scanWineLabel(image) {
  // `null` skips a destructuring default; the service already handles it.
  return cellarScan.scanWineLabel(image ?? undefined);
}

// Convenience wrapper for the multi-wine tasting-card scan (#139).
export async function scanTastingCard(image) {
  return cellarScan.scanTastingCard(image ?? undefined);
}
