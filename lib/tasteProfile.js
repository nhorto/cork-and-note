...
// The chat Edge Function accepts at most 8,000 characters per message. Keep a
// little headroom here so a large journal can never be rejected at dispatch.
// The report still uses aggregates from the whole journal; only the bounded
// evidence sample is reduced when verbose tasting details fill the prompt.
export const MAX_TASTE_MESSAGE_CHARS = 7_500;
const MAX_TASTE_EVIDENCE = 40;
const MIN_TASTE_EVIDENCE = MIN_DISTINCT_FOR_REPORT;
...
function evidenceLine(w) {
  const name = promptText(w.name, 80);
  const year = promptText(w.year, 10);
  const producer = promptText(w.producer, 80);
  const type = promptText(w.type, 40);
  const varietals = (Array.isArray(w.varietals) ? w.varietals : [])
    .slice(0, 3)
    .map((v) => promptText(v, 50))
    .filter(Boolean);
  const flavors = (Array.isArray(w.flavors) ? w.flavors : [])
    .slice(0, 6)
    .map((f) => promptText(f?.name, 40))
    .filter(Boolean);
  const bits = [
    `id ${w.id}`,
    [name, year].filter(Boolean).join(' ') || 'Unnamed wine',
    producer ? `by ${producer}` : null,
    [type, varietals.join(' / ')].filter(Boolean).join(', ') || null,
    `rated ${w.rating}/5`,
    sliderText(w.sliders),
    flavors.length ? `flavors: ${flavors.join(', ')}` : null,
    w.notes ? `note: "${promptText(w.notes, 120)}"` : null,
...
export function buildTasteUserMessage({ aggregates, evidence }) {
  ...
  a.byType
    .slice(0, 12)
    .forEach((t) => lines.push(`- ${promptText(t.type, 50)}: ${t.count}, avg ${t.avgRating}`));
  lines.push('');
  lines.push('By grape (count, average rating; a grape marked * has 2+ tastings averaging 4 or more):');
  a.byVarietal
    .slice(0, 12)
    .forEach((v) => lines.push(`- ${promptText(v.varietal, 60)}${v.standsOut ? ' *' : ''}: ${v.count}, avg ${v.avgRating}`));
  lines.push('');
  const s = a.sliders;
  lines.push(
  ...
  if (a.topFlavors.length) {
    lines.push(
      `Flavor notes tagged on wines rated 4 or higher: ${a.topFlavors
        .map((f) => `${promptText(f.name, 40)} (${f.count})`)
        .join(', ')}`
    );
  } else {
...
/**
 * Build a request that stays within the chat endpoint's per-message limit.
 * Re-running the sampler at each smaller size retains its best/newest/grape
 * mix instead of simply chopping the lowest-rated lines off the end.
 */
export function buildTasteRequest({ rated, aggregates }) {
  const list = Array.isArray(rated) ? rated : [];
  let evidence = pickEvidenceSample(list, MAX_TASTE_EVIDENCE);
  let message = buildTasteUserMessage({ aggregates, evidence });

  while (message.length > MAX_TASTE_MESSAGE_CHARS && evidence.length > MIN_TASTE_EVIDENCE) {
    evidence = pickEvidenceSample(list, evidence.length - 1);
    message = buildTasteUserMessage({ aggregates, evidence });
  }
...
export const tasteReportService = {
  ...
  async generate({ rated, now = Date.now() }) {
    try {
      ...
      const { evidence, message } = buildTasteRequest({ rated: list, aggregates });
