...
export const FREE_TIER_LIMITS = {
  ...
  chat: 5,
  tonights_pick: 0,
  // Guided Pro tools (owner decision 2026-09-11): Pro-only, free sees a sample.
  wine_list_scan: 0,
  wine_list_pick: 0,
  taste_report: 0,
  trip_plan: 0,
...
/**
 * The tasks that are Pro-only outright (free allowance 0). Screens use this to
 * decide "show the sample and an upgrade button" versus "count down a meter".
 */
export const PRO_ONLY_TASKS = Object.keys(FREE_TIER_LIMITS).filter(
  (task) => FREE_TIER_LIMITS[task] === 0
);
...
// ── Store identity ─────────────────────────────────────────────────────────
// Billing copy must name the store the user is actually on: an Android user
// told to cancel in their "Apple Account settings" has been told to cancel
// somewhere that does not exist. Every billing surface reads these two rather
// than hardcoding Apple.

/** What the user's billing account is called on this platform. */
export const STORE_ACCOUNT_NAME =
  Platform.OS === 'android' ? 'Google Play account' : 'Apple Account';

/**
 * Where this platform manages/cancels subscriptions. Play also supports a
 * per-product deep link (…/subscriptions?sku=<productId>&package=<app id>);
 * switch to it once the Play subscription products exist and we know which
 * sku the user holds — until then the plain page is the correct target.
 */
