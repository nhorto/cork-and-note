...
import { parseVarietals, varietalText } from './varietals';
import { wineriesService } from './wineries';

// Was this session an actual visit to a winery? Two kinds of row carry a
// winery but are NOT a visit to it: a location-optional log has no winery_id
// at all, and a bottle opened at home keeps the producer's winery_id with a
// null place_type (#294). Legacy rows were backfilled to place_type 'winery'
// by 20260608010000_logging_optional_location.sql, so requiring 'winery' here
// is safe for old data.
export function isWineryVisit(visit) {
  return Boolean(visit?.winery_id) && visit?.place_type === 'winery';
}
...
export const visitsService = {
  ...
  async createVisit(visitData) {
    try {
      ...
      // Resolve a winery row for winery-type places so the log shows up on the
      // map and in "your places". An existing winery passes through unchanged; a
      // freshly typed winery name is found-or-created (owned by this user). If
      // resolution fails (e.g. offline), fall back to a location-less log rather
      // than blocking the save.
      let wineryId = visitData.wineryId ?? null;
      if (!wineryId && visitData.placeType === 'winery' && (visitData.placeName || '').trim()) {
        const wineryResult = await wineriesService.findOrCreateWinery({
          name: visitData.placeName,
          latitude: visitData.latitude ?? null,
          longitude: visitData.longitude ?? null,
          directoryId: visitData.directoryId ?? null,
        });
        if (wineryResult.success && wineryResult.winery) {
          wineryId = wineryResult.winery.id;
        }
      }

      // Upload every wine's photos BEFORE the write. Object storage isn't
      // transactional, so photos are resolved to URLs first and the database
      // work then happens in one atomic call (§2.3).
      const winesPayload = [];
      let notesFailed = 0;
      for (const wine of visitData.wines || []) {
        ...
        if (wine.photos && wine.photos.length > 0) {
          ...
          photosFailed += wine.photos.length - winePhotoUrls.length;
        }

        winesPayload.push({
          winemaker: wine.winemaker ?? null,
          wine_name: wine.name || varietalText(wine.varietal) || 'Wine',
          wine_type: wine.type ?? null,
          wine_varietal: parseVarietals(wine.varietal),
          wine_year: wine.year ?? null,
          overall_rating: wine.overallRating || 0,
          sweetness: wine.ratings?.sweetness || 0,
          tannin: wine.ratings?.tannins || 0,
          acidity: wine.ratings?.acidity || 0,
          body: wine.ratings?.body || 0,
          alcohol: wine.ratings?.alcohol || 0,
          additional_notes: wine.additionalNotes ?? null,
          photo_url: JSON.stringify(winePhotoUrls),
          flavor_notes: wine.flavorNotes || [],
        });
      }

      // One transaction: the visit and every wine land together, or nothing
      // does. Previously this was N+ separate round trips, so a dropped
      // connection mid-save (routine in a tasting room) left a "ghost" visit
      // with some or none of its wines.
      const { data: result, error: rpcError } = await supabase.rpc(
        'create_visit_with_wines',
        {
          p_visit: {
            winery_id: wineryId,
            place_type: visitData.placeType ?? (wineryId ? 'winery' : null),
            place_name: visitData.placeName ?? null,
            latitude: visitData.latitude ?? null,
            longitude: visitData.longitude ?? null,
            visit_date: visitData.date,
            notes: visitData.notes || visitData.additionalNotes || null,
      ...
      if (rpcError) {
        console.error('Visit creation error:', rpcError);
        throw rpcError;
      }

      notesFailed = result?.notes_failed ?? 0;

      // The write is committed at this point, so refresh the derived views
      // before the read-back — otherwise a failed read-back would leave the
      // new tasting invisible until the next manual refresh.
      invalidate(CACHE_KEYS.visits, CACHE_KEYS.visitStats);

      // Read the visit back so callers keep receiving the full row.
      ...
      if (readError) {
        // The write succeeded; only the read-back failed. Report success with
        // the id we know rather than letting the caller think the save failed.
        console.error('Visit read-back error:', readError);
        return {
          success: true,
      ...
      return { success: true, visit, photosFailed, notesFailed };
    } catch (error) {
    ...
    try {
      ...
      const { data: wine, error: wineError } = await supabase
        ...
        .insert({
          ...
          wine_name: wineData.name || varietalText(wineData.varietal) || 'Wine',
          wine_type: wineData.type ?? null,
          wine_varietal: parseVarietals(wineData.varietal),
      ...
      if (wineData.flavorNotes && wineData.flavorNotes.length > 0) {
        notesFailed = await this.createWineFlavorNotes(wine.id, wineData.flavorNotes);
      }

      return { wine, photosFailed, notesFailed };
    } catch (error) {
  ...
  //
  // Returns the number of selected notes that could NOT be saved (0 = all good),
  // mirroring the photosFailed pattern, so callers can warn instead of reporting
  // success while data was dropped. (Audit: the missing-INSERT-policy 42501 error
  // used to be swallowed here — see 20260705010000_flavor_notes_insert_policy.sql.)
  async createWineFlavorNotes(wineId, flavorNotes) {
    try {
      ...
      if (missingNames.length > 0) {
        ...
        // If the upsert above failed, the custom names simply won't come back
        // and are counted as dropped below. If the re-fetch itself fails, fall
        // through with the original catalog matches so those still get linked.
      ...
      // Anything that never made it into the catalog (e.g. blocked upsert).
      return flavorNotes.length - allNotes.length;
    } catch (error) {
      console.error('Error creating wine flavor notes:', error);
      return flavorNotes?.length ?? 0;
    }
  },
  ...
  async _getUserVisits() {
    try {
      ...
      const visitsWithPhotos = visits.map(visit => ({
        ...visit,
        // Flatten the directory's state onto the winery. It is the only
        // authoritative source of "which state is this place in" (#295 needs it
        // for the Places badges), and a nested object two levels down is
        // awkward for every consumer.
        wineries: visit.wineries
          ? { ...visit.wineries, state: visit.wineries.winery_directory?.state ?? null }
          : visit.wineries,
        // Parse visit photos
  ...
  async updateVisit(visitId, visitData) {
    try {
      ...
      // Same winery resolution as createVisit: editing a log onto a winery place
      // typed by name (no saved winery) should find-or-create that winery so it
      // appears on the map. Only kicks in when a winery place is actually set.
      let resolvedWineryId = visitData.wineryId;
      if (
        (visitData.wineryId === undefined || visitData.wineryId === null) &&
        visitData.placeType === 'winery' &&
        (visitData.placeName || '').trim()
      ) {
        const wineryResult = await wineriesService.findOrCreateWinery({
          name: visitData.placeName,
          latitude: visitData.latitude ?? null,
        ...
        if (wineryResult.success && wineryResult.winery) {
          resolvedWineryId = wineryResult.winery.id;
        }
      }

      // Mirror createVisit's place fields so editing can add/change/clear a place.
      const placeUpdate = {};
      if (resolvedWineryId !== undefined) placeUpdate.winery_id = resolvedWineryId ?? null;
  ...
  async updateWine(wineId, wineData) {
    try {
      ...
      if (wineData.flavorNotes !== undefined) {
        // Delete existing flavor notes. Checked: if this fails and we insert
        // anyway, the unique(wine_id, flavor_note_id) constraint rejects the
        // re-insert and the wine keeps a stale/merged set while reporting success.
        ...
        if (deleteNotesError) {
          console.error('Error clearing wine flavor notes:', deleteNotesError);
          throw deleteNotesError;
        }

        // Add new flavor notes
        if (wineData.flavorNotes.length > 0) {
          notesFailed = await this.createWineFlavorNotes(wineId, wineData.flavorNotes);
        }
      }

      // Wine data is nested inside the cached visits list.
      invalidate(CACHE_KEYS.visits);
      return { success: true, wine, photosFailed, notesFailed };
    } catch (error) {
  ...
  async _getVisitStats() {
    try {
      ...
      const { data: visits, error: visitsError } = await supabase
        .from('visits')
        .select('id, winery_id, place_type')
        .eq('user_id', user.id);
      if (visitsError) throw visitsError;

      const visitIds = (visits || []).map(v => v.id);
      const totalVisits = visitIds.length;
      // "Places" = distinct wineries actually visited. Location-optional logs
      // have a null winery_id, and a bottle opened at home carries the
      // producer's winery_id with a null place_type (#294); neither is a place.
      const totalWineries = new Set(
        (visits || []).filter(isWineryVisit).map(v => v.winery_id)
      ).size;
  ...
  // Derive Home's counters from the visits payload it already fetched. Keeping
  // this pure avoids a second auth check + database round trip there, because
  // getUserVisits is also required for Home's recents and highlights.
  // Location-less tasting logs still count as visits/wines, but not places.
  summarizeVisitStats(visits) {
    const list = Array.isArray(visits) ? visits : [];

    return {
      totalVisits: list.length,
      totalWineries: new Set(list.filter(isWineryVisit).map(v => v.winery_id)).size,
      totalWines: list.reduce(
        (total, visit) => total + (Array.isArray(visit.wines) ? visit.wines.length : 0),
        0
      ),
    };
  },
  ...
  // place (see #99), and so are cellar-origin tastings (#294).
  summarizeVisits(visits) {
    const placed = (visits || []).filter(v => isWineryVisit(v) && v.wineries?.name);
