...
export function buildFacts({ visits = [], bottles = [], consumptions = [] } = {}) {
  ...
  for (const pour of pours) {
    // Gifts, sales, spoilage and Coravin samples do not mean a bottle was
    // opened. Only the consumed reason earns opening and drink-window badges.
    if (pour?.reason !== 'consumed') continue;
    bottlesOpened += 1;
